...
Backtest::Backtest(TD& td,
                   const std::vector<SymbolInfo>& symbols,
                   bool bt_enabled)
    : td{td}, enabled{bt_enabled} {
  if (!enabled)
    return;
...
std::vector<Candle> Backtest::time_series(const std::string& symbol,
                                          int n_days) {
...
Candle Backtest::real_time(const std::string& symbol) {
